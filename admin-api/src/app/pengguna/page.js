...
export default function HalamanPengguna() {
  ...
  const fetchUsers = async (search = '') => {
    setLoading(true);
    try {
      const res = await fetch(`/api/pengguna?search=${encodeURIComponent(search)}`);
  ...
  return (
    <AdminLayout>
      <div className="min-h-screen bg-black p-6">
        ...
        {loading && <div className="text-center py-20 text-xl text-gray-400">Loading...</div>}
        {error && <div className="text-center py-20 text-xl text-red-500">Error: {error}</div>}
          ...
          {users.map((user) => (
            <div key={user.uid} className="group bg-gradient-to-b from-gray-900 to-black border-2 border-gray-800 rounded-3xl p-6 hover:border-red-600 transition-all duration-500 shadow-2xl hover:shadow-red-600/30">
              ...
              <div className="flex items-center justify-between mb-5">
                <img src={user.foto} alt={user.nama} className="w-20 h-20 rounded-full object-cover ring-4 ring-red-600 group-hover:ring-red-400 transition-all" onError={(e) => e.target.src = '/logo_kecil.png'} />
                <span className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider ${user.status === 'Aktif' ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-400'}`}>
                  {user.status}
                </span>
              </div>
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between"><span className="text-gray-500">Dibuat</span><span className="text-gray-300">{user.dibuat}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Login terakhir</span><span className="text-gray-300">{user.terakhirLogin}</span></div>
              </div>
              <div className="mt-8 flex gap-3">
                <button className="flex-1 bg-yellow-600 hover:bg-yellow-500 text-black font-bold py-3 rounded-2xl transition transform hover:scale-105">Edit</button>
                <button className="flex-1 bg-red-600 hover:bg-red-500 text-white font-bold py-3 rounded-2xl transition transform hover:scale-105">Hapus</button>
              </div>
            </div>
          ))}
        </div>
