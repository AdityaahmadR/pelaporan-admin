...
export default async function DetailLaporan({ params }) {
  ...
  const gambarMatch = laporan.isi_laporan?.match(/(https?:\/\/[^\s]+\.(jpg|jpeg|png|webp|gif))/i);
  const gambarUrl = gambarMatch ? gambarMatch[0] : null;

  // Bersihkan link gambar dari teks
  const isiBersih = (laporan.isi_laporan || '')
        ...
        <div className={detailStyles.container}>
          <div className={detailStyles.header}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
              <Link href="/app" className={detailStyles.backButton}>Back</Link>
          ...
          <div className={detailStyles.pelaporCard}>
            ...
            <div className={detailStyles.waktu}>
              ...
              <br />
              <strong>
                {new Date(laporan.tanggal).toLocaleTimeString('id-ID', {
